    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-900 via-amber-800 to-amber-900">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl font-bold text-amber-400">欢迎, {profile?.username || '用户'}</h2>
            <p className="text-amber-200">
              {profile?.role === 'admin' ? '管理员' : '普通用户'}
            </p>
            <button
              onClick={() => setShowDebug(!showDebug)}
              className="text-amber-300 text-sm underline mt-2"
            >
              显示调试信息
            </button>
          </div>
        ...
        {showDebug && (
          <div className="bg-amber-900/50 p-4 rounded-lg mb-6 text-amber-200 text-sm">
            <p><strong>调试信息:</strong></p>
            <p>用户ID: {user?.id || '无'}</p>
            <p>邮箱: {user?.email || '无'}</p>
            <p>用户名: {profile?.username || '无'}</p>
            <p>角色: {profile?.role || '无'}</p>
            <button
              onClick={handleForceLogout}
